...
export const withSubscriptions = Prisma.defineExtension((client) =>
  client.$extends({
    name: 'subscriptions',
    model: {
      user: {
        eventStream: (user_id: string | undefined) => {
          ...
          return withAsyncIteratorFilter(
            () =>
              pubsub.asyncIterator<{ user_id: string; event: string }>(`user_${user_id}`),
            (e) => e.user_id === user_id
          );
        },
      },
      ...
      camp: {
        eventStream: (camp_id: string) => {
          ...
          return withAsyncIteratorFilter(
            () =>
              pubsub.asyncIterator<{ camp_id: string; event: string }>(`camp_${camp_id}`),
            (e) => e.camp_id === camp_id
          );
        },
      },
