 ...
 *
 */
export const passedValidUsername = inputRuleWithContext('passedValidUsername')(
  (yup, ctx) =>
    yup.object({
      username: yup
        .string()
...
export const isValidCampInput = inputRuleWithContext('validateCampInput')(
  (yup) =>
    yup.object({
      name: yup
        .string()
        .min(3, 'Camp name must be at least 3 characters long')
        .max(50, 'Camp name cannot exceed 50 characters'),
      description: yup.string().max(255, 'Camp description cannot exceed 255 characters'),
    }),
  { abortEarly: true }
);

export const hasValidUUID = inputRuleWithContext()(
  (yup) => {
    return yup.object({
      id: yup.string().uuid('Camp ID must be a valid UUID'),
    });
  },
  { abortEarly: true }
);
